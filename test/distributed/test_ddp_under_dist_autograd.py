...
class Trainer:
    ...
    def do_backward(self, mini_batch: FeatureSet):
        ...
        gLogger.info(f"Grads dict: {grads_dict}")
        return (
...
def get_training_examples():
    ...
    idx = 0
    # Every example has another one with an opposite label.
    for label in (0, 1):
        for x in range(2):
            for y in range(2):
                for z in range(2):
                    ...
                    training_examples.labels[idx] = label
                    idx += 1
        ...
        for future in futures:
            ...
            for grad in ddp_grads:
                self.assertAlmostEqual(
                    0.0,
                    grads.norm(),
                    "The grad for any ddp parameter should be zeros, because "
            ...
            for grad in non_ddp_grads:
                self.assertNotAlmostEqual(
                    0.0,
                    grads.norm(),
                    "The grad for any non-ddp parameter shouldn't be zeros",
                )
